...
public class ReviewDAO {
	...
	public List<ReviewVO> selectReview(int stno, int memno) throws SQLException{
	      Connection con=null;
	      PreparedStatement ps=null;
	      ResultSet rs=null;
	      ...
	      try {
	         ...
	         String sql=" select re_review from s2_review"
	         + " where st_no=? and MEM_NO=?";
	         ps=con.prepareStatement(sql);
	...
	public List<ReviewVO> selectReview(int stno) throws SQLException{
		...
		try {
			...
			+ " where st_no=? order by re_regdate desc";
			ps=con.prepareStatement(sql);
	...
	/*
	 * public ReviewVO selectReview(int stno) throws SQLException { Connection
	 * con=null; PreparedStatement ps=null; ResultSet rs=null;
	 * 
	 * ReviewVO vo = new ReviewVO(); try { //1,2 con=pool.getConnection();
	 * 
	 * //3 String sql=" select * from s2_review" + " where st_no=?";
	 * ps=con.prepareStatement(sql); ps.setInt(1, stno);
	 * 
	 * //4 rs=ps.executeQuery(); if(rs.next()) { vo.setStno(stno);
	 * vo.setReview(rs.getString("RE_REVIEW")); vo.setScore(rs.getInt("RE_SCORE"));
	 * vo.setRegdate(rs.getTimestamp("RE_REGDATE"));
	 * vo.setLike(rs.getInt("RE_LIKE")); }
	 * 
	 * System.out.println("리뷰 결과 vo="+vo+", 매개변수 stno="+stno);
	 * 
	 * return vo; }finally { pool.dbClose(rs, ps, con); } }
	 */
	...
	public int insertReview(ReviewVO vo) throws SQLException {
		...
		try {
			...
			String sql="insert into s2_review(RE_NO, ST_NO, MEM_NO, RE_REVIEW, RE_SCORE, RE_REGDATE, RE_LIKE)"
					+ " values(s2_review_seq.nextval,?,?,?,?,sysdate,0)";
	...
	public int deleteReview(int rno, int sno) throws SQLException {
		Connection con=null;
		PreparedStatement ps=null;
		...
		try {
			...
			String sql="delete s2_review where RE_NO=? and ST_NO=?";
			ps=con.prepareStatement(sql);
			ps.setInt(1, rno);
			ps.setInt(2, sno);
